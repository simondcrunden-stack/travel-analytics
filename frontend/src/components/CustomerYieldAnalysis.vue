  ...
  <div class="bg-white rounded-xl shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-pink-50">
      <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
        <span class="mdi mdi-account-star text-purple-600"></span>
        Customer Yield Analysis
      </h3>
      <p class="text-sm text-gray-600 mt-1">Revenue and booking metrics by customer/traveller</p>
    </div>
    ...
    <div v-else-if="data" class="p-6">
      ...
      <div class="mb-4">
        <div class="relative">
          ...
          <input
            ...
            placeholder="Search travellers by name or email..."
          />
        </div>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" @click="sortBy('traveller_name')">
                Customer
                <span v-if="sortField === 'traveller_name'" class="mdi" :class="sortAscending ? 'mdi-arrow-up' : 'mdi-arrow-down'"></span>
              </th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="customer in sortedCustomers" :key="customer.traveller_id" class="hover:bg-gray-50">
              <td class="px-4 py-3 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-8 w-8 bg-purple-100 rounded-full flex items-center justify-center">
                    <span class="text-purple-600 font-semibold text-sm">{{ getInitials(customer.traveller_name) }}</span>
                  </div>
                  <div class="ml-3">
                    <div class="text-sm font-medium text-gray-900">{{ customer.traveller_name }}</div>
                    <div class="text-xs text-gray-500">{{ customer.traveller_email }}</div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-4 py-3 whitespace-nowrap text-right text-sm text-gray-900">
                {{ formatCurrency(customer.total_booking_value) }}
              </td>
              ...
              <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium" :class="getOnlinePercentageClass(customer.online_percentage)">
                  {{ customer.online_percentage }}%
                </span>
              </td>
              <td class="px-4 py-3 whitespace-nowrap text-right text-sm text-gray-900">
                {{ customer.modification_percentage }}%
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <!-- No Data -->
      <div v-if="!data.customers || data.customers.length === 0" class="text-center py-12">
        <span class="mdi mdi-account-off text-6xl text-gray-300"></span>
        <p class="mt-2 text-gray-500">No customer data available for the selected period</p>
      </div>
    </div>
  </div>
</template>
...
const loadData = async () => {
  ...
  try {
    ...
    console.error('Error loading customer yield data:', err)
    error.value = 'Failed to load customer yield analysis data'
  } finally {
    loading.value = false
  }
}
...
const sortedCustomers = computed(() => {
  ...
  let customers = [...data.value.customers]

  // Filter by search query
  if (searchQuery.value) {
    const query = searchQuery.value.toLowerCase()
    customers = customers.filter(customer => {
      const name = (customer.traveller_name || '').toLowerCase()
      const email = (customer.traveller_email || '').toLowerCase()
